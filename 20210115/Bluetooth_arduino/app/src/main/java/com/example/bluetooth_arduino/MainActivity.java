...
// ------------------------------------------------------------------------------------------------
    ...
    // (메시지 상태를 식별하기 위해 블루투스 핸들러에서 사용)
    private final static int MESSAGE_READ = 2;
    // used in bluetooth handler to identify message update
    // (메시지 업데이트를 식별하기 위해 블루투스 핸들러에서 사용)

    // --------------------------------------------------------------------------------------------
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState); // AppCompatActivity에 있는 onCreate() 호출
        setContentView(R.layout.activity_main); // 레이아웃을 보여줌

        // UI Initialization (UI 초기화)
        final Button connectButton = findViewById(R.id.connectButton); // connect 버튼
        final ProgressBar progressBar = findViewById(R.id.progressBar); // 버퍼링 아이콘
        progressBar.setVisibility(View.GONE); // VISIBLE(보이고), INVISIBLE(안보이고), GONE(없어진)
        final ImageView imageView = findViewById(R.id.imageView); // 버퍼링 아이콘
        imageView.setVisibility(View.VISIBLE); // VISIBLE(보이고), INVISIBLE(안보이고), GONE(없어진)
        final ImageView imageView2 = findViewById(R.id.imageView2); // 버퍼링 아이콘
        imageView2.setVisibility(View.GONE); // VISIBLE(보이고), INVISIBLE(안보이고), GONE(없어진)
        final ImageView imageView3 = findViewById(R.id.imageView3); // 버퍼링 아이콘
        imageView3.setVisibility(View.VISIBLE); // VISIBLE(보이고), INVISIBLE(안보이고), GONE(없어진)
        final ImageView imageView4 = findViewById(R.id.imageView4); // 버퍼링 아이콘
        imageView4.setVisibility(View.VISIBLE); // VISIBLE(보이고), INVISIBLE(안보이고), GONE(없어진)
        final TextView textView = findViewById(R.id.textView);
        final Button RedButton = findViewById(R.id.RedButton); // ON 버튼
        final Button BlueButton = findViewById(R.id.BlueButton); // OFF 버튼
        final Button statusButton = findViewById(R.id.statusButton); // status 버튼
        RedButton.setEnabled(false); // 시작시 ON 버튼 비활성화(블루투스 연결시에만 활성화)
        BlueButton.setEnabled(false); // 시작시 OFF 버튼 비활성화(블루투스 연결시에만 활성화)
        statusButton.setEnabled(false); // 시작시 status 버튼 비활성화(블루투스 연결시에만 활성화)

        // SelectDeviceActivity에서 블루투스 장치를 선택한 경우
        // Intent : 두 액티비티간 데이터를 주고 받을 때 사용
        ...
        if (deviceName != null){ // 데이터를 가져올 디바이스가 있다면
            ...
            progressBar.setVisibility(View.VISIBLE); // 버퍼링 아이콘 보이기
            connectButton.setEnabled(false); // connect 버튼 보이게 // 비활성화(False)
            ...
            */
            ...
            만약 null을 반환하면 스마트폰에서 블루투스 를 지원하지 않는 것입니다.
             */
            ...
            public void handleMessage(Message msg){ // handler를 통해 실행결과를 Message로 전달
                switch (msg.what){ // message의 what값을 이용. 알맞게 UI처리
                    ...
                    case CONNECTING_STATUS: // 1, 연결된 상태일 때. 메세지 상태 식별
                        switch(msg.arg1){ // arg : 간단한 정수 값을 저장할 수 있는 멤버 변수
                            case 1: // 연결 성공
                                ...
                                progressBar.setVisibility(View.GONE); // 버퍼링 아이콘 없어짐
                                connectButton.setEnabled(true); // connect 버튼 활성화
                                RedButton.setEnabled(true); // ON 버튼 활성화
                                BlueButton.setEnabled(true); // OFF 버튼 활성화
                                statusButton.setEnabled(true); // status 버튼 활성화
                                break;
                            case -1: // 연결 실패
                    ...
                    case MESSAGE_READ: // 2, 메세지 업데이트 식별
                        ...
                        switch (arduinoMsg){ // toLowerCase(); : 문자열을 소문자로 변환해서 반환
                            case "1\r": // 아두이노의 Red led가 켜진 상태면
                                //textView.setText("Blue LED Status : " + arduinoMsg);
                                imageView.setVisibility(View.VISIBLE); // VISIBLE(보이고), INVISIBLE(안보이고), GONE(없어진)
                                break;
                            case "0\r": // 아두이노의 Red led가 꺼진 상태면
                                //textView.setText("Blue LED Status : " + arduinoMsg);
                                imageView2.setVisibility(View.VISIBLE); // VISIBLE(보이고), INVISIBLE(안보이고), GONE(없어진)
                                break;
                            default:
        ...
        RedButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                // cmdText = "w";
                cmdText = "null";
                String btnstate = RedButton.getText().toString().toLowerCase();
                switch (btnstate)
                {
                    case "led on":
        ...
        BlueButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                // cmdText = "s";
                cmdText = "null";
                String btnstate = BlueButton.getText().toString().toLowerCase();
                switch (btnstate)
                {
                    case "led on":
    ...
    public static class CreateConnectThread extends Thread {
        ...
        public void run() {
            ...
            // 그렇지 않으면 연결 속도가 느려지므로 검색을 취소하십시오.
    ...
    public static class ConnectedThread extends Thread {
        ...
        // 기본 활동에서 이것을 호출하여 데이터를 원격 장치로 보냅니다.
