...
// ------------------------------------------------------------------------------------------------
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final Button onButton = findViewById(R.id.onButton); // ON 버튼
        final Button offButton = findViewById(R.id.offButton); // OFF 버튼
        final Button statusButton = findViewById(R.id.statusButton); // status 버튼
        onButton.setEnabled(false); // 시작시 ON 버튼 비활성화(블루투스 연결시에만 활성화)
        offButton.setEnabled(false); // 시작시 OFF 버튼 비활성화(블루투스 연결시에만 활성화)
                ...
                switch (msg.what){ // message의 what값을 이용. 알맞게 UI처리
                    ...
                    case CONNECTING_STATUS: // 1, 연결된 상태일 때. 메세지 상태 식별
                        switch(msg.arg1){ // arg : 간단한 정수 값을 저장할 수 있는 멤버 변수
                            case 1: // 연결 성공
                                ...
                                onButton.setEnabled(true); // ON 버튼 활성화
                                offButton.setEnabled(true); // OFF 버튼 활성화
                    ...
                    case MESSAGE_READ: // 2, 메세지 업데이트 식별
                        ...
                        switch (arduinoMsg){ // toLowerCase(); : 문자열을 소문자로 변환해서 반환
                            case "1\r": // 아두이노의 led가 켜진 상태면
                                textView.setText("Blue LED Status : " + arduinoMsg);
                                break;
                            case "0\r": // 아두이노의 led가 꺼진 상태면
                                textView.setText("Blue LED Status : " + arduinoMsg);
                                break;
                            default:
