...
// --------------------------------------------------------------------------------------------
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Toast.makeText(getApplicationContext(), "블루투스 연결을 먼저 해주세요.", Toast.LENGTH_LONG).show();
        ...
        if (deviceName != null) { // 데이터를 가져올 디바이스가 있다면
            ...
            bluetooth.setEnabled(false); // connect 버튼 비활성화
            ...
            pbtn.setOnClickListener(new Button.OnClickListener(){
                ...
                public void onClick(View view)
                {
                    ...
                    //count++;
                    num++;
                    settingNum.setText(""+num);
                    //settingNum.setText(a+count);
                    //settingNum.setText(count);
                    cmdText = Integer.toString(num)+'\r'; // 정수를 문자열로 형변환
                    connectedThread.write(cmdText);

                    if (count > 50)
                    {
                        Toast.makeText(getApplicationContext(), "온도를 낮추세요.", Toast.LENGTH_LONG).show();
                    }
                }
            });
            ...
            settingNum.addTextChangedListener(new TextWatcher() {
                @Override // 변경되기전 문자열을 담고있다.
                public void beforeTextChanged(CharSequence s, int start, int count, int after) {
                    Edit = s.toString();
                }

                @Override // 텍스트가 변경될 때마다 호출된다. 보통 여기서 이벤트 작업을 많이 함
                public void onTextChanged(CharSequence s, int start, int before, int count)
                {   // editText에 포커스가 되어있고 텍스트가 하나라도 입력되어 있을때 동작하기 위해서 추가.
                    if (settingNum.isFocusable() && !s.toString().equals("")) {
                        try {
                            num = Integer.parseInt(settingNum.getText().toString());
                            cmdText = Integer.toString(num)+'\r'; // 정수를 문자열로 형변환
                            connectedThread.write(cmdText);
                    ...
                    case CONNECTING_STATUS: // 1, 연결된 상태일 때. 메세지 상태 식별
                        switch (msg.arg1) { // arg : 간단한 정수 값을 저장할 수 있는 멤버 변수
                            case 1: // 연결 성공
                                ...
                                bluetooth.setEnabled(true); // connect 버튼 활성화
                                break;
                            case -1: // 연결 실패
                                textView.setText("연결 실패");
                                bluetooth.setEnabled(true); // connect 버튼 활성화
                                break;
                        }
                        break;
